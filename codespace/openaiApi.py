...
def parse_input_data(data_str):
    """
    解析输入数据，处理包含annotations字段的JSON格式
    """
    try:
        ...
        # 如果数据包含annotations字段，提取annotations
        if isinstance(data_obj, dict) and 'annotations' in data_obj:
        ...
        # 如果是单个设备对象
            ...
            print(f"无法识别的数据格式: {type(data_obj)}")
            print(f"数据键: {data_obj.keys() if isinstance(data_obj, dict) else 'N/A'}")
            return []
    except Exception as e:
        print(f"数据解析错误: {e}")
        return []

def create_id_label_mapping(input_data):
    """
    从输入数据创建ID到标签的映射
    """
    id_to_label = {}
    try:
        devices = parse_input_data(input_data)
        for device in devices:
            if 'id' in device and 'label' in device:
                id_to_label[device['id']] = device['label']
    except Exception as e:
        print(f"创建ID标签映射时出错: {e}")
    
    print(f"创建了 {len(id_to_label)} 个设备的ID-标签映射")
    return id_to_label

def visualize_comparison(input_data, output_data, title="设备位置对比"):
    """
    可视化输入和输出设备位置的对比
    专门处理带annotations字段的输入格式
    """
    ...
    # 创建ID到标签的映射
    id_to_label = create_id_label_mapping(input_data)
    
    def plot_devices(ax, data, title_suffix):
        ...
        try:
            ...
            print(f"{title_suffix}: 找到 {len(devices)} 个设备")
            
            device_count = {}
            
            for device in devices:
                if 'points' in device and device['points']:
                    points = np.array(device['points'])
                    
                    # 获取标签 - 优先使用设备自带的label，否则从映射中查找
                    if 'label' in device:
                        label = device['label']
                    elif 'id' in device and device['id'] in id_to_label:
                        label = id_to_label[device['id']]
                    else:
                        label = 'unknown'
                    
                    color = device_colors.get(label, 'black')
                    
                    # 统计设备类型
                    device_count[label] = device_count.get(label, 0) + 1
            ...
            # 打印设备统计
            print(f"{title_suffix}设备统计:")
            for label, count in device_count.items():
                print(f"  {label}: {count}个")
                        
        except Exception as e:
            print(f"{title_suffix}数据解析错误: {str(e)}")
            ax.text(0.5, 0.5, f'数据解析错误: {str(e)}', 
    ...
    # 绘制输入和输出
    plot_devices(ax1, input_data, '治理前')
    plot_devices(ax2, output_data, '治理后')
    
    plt.tight_layout()
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    filename = f'设备位置对比_{timestamp}.png'
...
try:
    ...
    try:
        ...
        if '[' in response_content:
            ...
            # 创建对比可视化
            print(f"\n=== 创建对比可视化 (共{len(output_devices)}个设备) ===")
            visualize_comparison(content, output_devices, "电网设备优化对比")

...
def main3(url, cookie_2=None, psrId=None):
    for i in range(10):
        try:
...
def output(body_data, psrId):
    ...
    for k, v in line_data.items():
        ...
        for i in range(len(v)):
            ...
            if idx == 0:
                line_coordinates[:] = [line_coordinates[0], [line_coordinates[0][0], line_coordinates[0][1] - 0.00002 * (i + 1) * symbol]]
            else:
                line_coordinates[:] = [[line_coordinates[idx][0], line_coordinates[idx][1] - 0.00002 * (i + 1) * symbol], line_coordinates[idx]]
            point_coordinates[:2] = [line_coordinates[idx][0], line_coordinates[idx][1] - 0.00002 * (i + 1) * symbol]

    with open(geo2Path, 'w', encoding='utf-8') as current_data2:
